...
- **v1.1 — 2026-05-16, present commit** — **Step 2 shipped.** New module `substrate/brand_voice.py` with `read_voice()`, `save_voice()` (auto-bumps `profile_version`), and `voice_prompt_block()` (text block injected into NIS prompts). `Module.BRAND_VOICE` enum added; `profile_revision` added to `STRATEGIC_FIELDS`. Two endpoints: `GET` and `POST /api/atlas/brand-voice`. Frontend: dedicated `Brand Voice` sidebar item under Core, 5-section editor (Positioning / Tone+Hero / Signature+Required / Banned / Examples), live "What the LLM sees" preview. `generate_content_llm()` now injects the structured voice block into every NIS prompt; Image → NIS inherits via the same function. Every save writes a `decision_event` with `module='brand_voice'`, `field_name='profile_revision'`. **The two-store split is closed for voice** (operational defaults like default_care still live in JSON; all voice is in `brand_profile`). 47 QA assertions, all 11 QA suites green. Step 3 (drift detector) remains blocked on edit-volume data.
