...
- **v2.1 — 2026-05-18 (latest), present commit** — M5 shipped: schema v9 + `content_benchmarks` table. Writer in `substrate/content_benchmarks.py` (`lock_benchmark` with per-(scope, scope_ref, type) cap of 3 active; `list_applicable` resolves seed candidates with scope priority asin > family_decision_class > family > global, tie-break by used_count then recency; `bump_usage` increments + stamps; `supersede`/`archive`/`reactivate` lifecycle; `flag_by_unknown` flips active benchmarks to `review_recommended` when an unknown they were locked against closes). `substrate/unknowns.resolve_unknown` now triggers `flag_by_unknown` as a best-effort side effect (import inside, swallows errors). 7 new endpoints under `/api/atlas/benchmarks/*` (list, lock, get, applicable, archive, reactivate, supersede, bump-usage). New sidebar item Benchmarks with lock form, applicable-preview resolver, and inbox table (status filter, archive/reactivate inline actions, review_reason inline). `qa_m5.py` ships 55 assertions; 15 prior tmp suites + qa_m2 + qa_m4 still green; 32/32 substrate unit tests green. NIS pipeline does not yet seed from benchmarks at generation time — that wiring is the next pass (M5b: thread `list_applicable` into the citation_chain prompt and `bump_usage` on selection).
- **v2.2 — 2026-05-18 (latest), present commit** — M5b shipped: benchmarks wired into the citation_chain. New helper `resolve_applicable_benchmarks(workspace_id, bundle, decision_class)` reads `bundle.factual.variation_family` and resolves up to three applicable benchmarks via the existing `content_benchmarks.list_applicable` scope-priority resolver. `build_cited_prompt` now accepts a `benchmarks` kwarg and injects a `PRIOR APPROVED PATTERNS` section after the unknowns block, with explicit instructions: treat Rank 1 as the strong default, match its structure/voice/key facts unless ASIN facts force divergence, and set the new top-level `seeded_from_benchmark` field in the JSON output (or `null` with `seed_divergence_reason`). The JSON output schema gains a `benchmark` value for the `layer` enum so citations can reference a benchmark_id directly. `verify_citations` now takes an optional `benchmark_ids` list and treats `layer='benchmark'` citations as verified when the cited id is in that list. `generate_cited` orchestrates the full path: resolve → inject → call LLM → parse → validate `seeded_from_benchmark` against the list we offered → bump_usage on the seeded benchmark (best-effort, swallowed) → record `applicable_benchmark_ids`, `seeded_from_benchmark`, and `seed_divergence_reason` in `bundle_summary` on the decision_event → return all three in the API response. Cited NIS UI gains a "Benchmark seeding" panel below confidence bars showing whether the generation seeded (with the picked benchmark highlighted via accent border) or diverged (with the reason), plus a collapsible list of applicable benchmarks offered. Drive-by fix in `substrate/context.py:_layer_competitor_state`: SELECT used the wrong column name (`asin_or_sku`) — it's `asin` in the v7 schema, and `value` is JSONB not numeric. Caught and logged before but never blocked behavior. `qa_m5b.py` ships 34 assertions covering decision_class→benchmark_type mapping, family-based resolution, asin override, unmapped/empty cases, prompt injection (presence/absence + key fields rendered), benchmark-aware verifier (4 status outcomes), and end-to-end generate_cited returning `applicable_benchmarks` and bump_usage contract. 15 prior tmp suites + qa_m2 + qa_m4 + qa_m5 still green; 32/32 substrate unit tests green. Phase 1.5 sprint M1+M2+M3+M4+M5+M5b is complete.
- **v2.3 — 2026-05-19, present commit** — M6 Day 1 shipped: catalog audit substrate + Ingest tab. Schema v10 adds 7 tables (`brand_workspace`, `audit_rules`, `cohort_classifications`, `catalog_audit_findings`, `audit_decisions`, `audit_sessions`, `analytics_views`). New writers: `substrate/brand_workspace.py` (workspace registry, soft-archive via `is_active`, `last_audit_at` touch), `substrate/audit_rules.py` (15 SEED_RULES library covering fewer_than_7_images, missing_a_plus_top_decile, orphan_variation, duplicate_style_group, abandoned_subcategory, missing_description, missing_bullets, fewer_than_3_reviews, below_bsr_floor, out_of_stock_90d, missing_country_of_origin, missing_care_instructions, missing_search_terms, parent_with_one_child, child_without_parent — operator-editable substrate, not config; `fork_for_brand` produces brand-specific overrides with idempotency on `(parent_rule_id, workspace_id, threshold_json)`; `resolve_rules_for_brand` returns the effective ruleset with brand overrides winning over globals), `substrate/catalog_audit.py` (cohort_classifications bulk + read with always-three-keys `count_by_cohort` envelope; findings write + summarize; decisions journal; audit_sessions for the per-session decision trail; pinned analytics views), `substrate/catalog_ingest.py` (XLSX → substrate engine: `parse_workbook` autodetects columns, `classify_active_cohort` applies the locked rule [active = TTM units ≥ 1 OR sessions ≥ 50, dormant if explicit no-sale, unknown otherwise], `compute_coverage` produces per-column fill rate, `detect_skipped_rules` returns rules that can't fire because their data dependency is < 5% filled or requires a connector that isn't wired). 8 new endpoints under `/api/atlas/`: workspaces (list, switch via cookie), audit-rules (list with optional `resolved=1`, get, edit, fork), catalog/ingest (multipart or filepath JSON; respects `preview_only=1`), catalog/coverage. Boot wiring in `app.py` auto-seeds the 15 rules + registers the `novelle` workspace on startup. Frontend: Catalog page restructured around 4 sub-tabs (Ingest / Audit / Analytics / Rules) with a topbar workspace switcher chip; Ingest sub-tab ships first (drag-drop dropzone + workspace_id input + preview-only toggle + ingest button + coverage tiles + skipped-rules list); Audit / Analytics / Rules render explicit stubs that name the Day-2/3/4 deliverable. The legacy Catalog Health uploader is preserved inside a collapsed details block under the Audit stub for backward compatibility (will be retired after M6 Day 2). `qa_m6_ingest.py` ships 30 assertions covering workspace register/list, 15-rule seed idempotency, brand fork override resolution, end-to-end Roxy ingest (38,844 ASINs, 4,987 active cohort), and the count_by_cohort envelope. All prior QA suites (qa_m2 70/70, qa_m4 63/63, qa_m5 55/55, qa_m5b 34/34) still green. M6 Day 2 (audit run + findings table + revenue-at-risk ranking) is next.
